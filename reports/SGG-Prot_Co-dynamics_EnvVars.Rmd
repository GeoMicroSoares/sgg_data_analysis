...
cor_genera_geochem = as(metadata_cor_main_genera[, c("Genus", "Abundance", "Site_name", "Month",
                                                     "Temp","SO4","DO","pH",
                                                     "Mn","Mg","Zn","Na","Li","Ba",
                                                     "B","As","Co","Cu","Se","X200_Hg","X201_Hg","X202_Hg")], "data.frame")
cor_genera_geochem$DO[is.na(cor_genera_geochem$DO)] <- 44.1
cor_genera_geochem$pH[is.na(cor_genera_geochem$pH)] <- 7
cor_genera_geochem=cor_genera_geochem[ , colSums(is.na(cor_genera_geochem)) == 0]
...
pearson_gal = head(cor_genera_geochem_gal_ggcorr$data, n=12)
pearson_gal$y = "Gallionella"
pearson_sid = head(cor_genera_geochem_sid_ggcorr$data, n=12)
pearson_sid$y = "Sideroxydans"
pearson_fer = head(cor_genera_geochem_fer_ggcorr$data, n=12)
pearson_fer$y = "Ferriphaselus"
pearson_sulfuri = head(cor_genera_geochem_sulfuri_ggcorr$data, n=12)
pearson_sulfuri$y = "Sulfuricurvum"
pearson_sulfuro = head(cor_genera_geochem_sulfuro_ggcorr$data, n=12)
pearson_sulfuro$y = "Sulfurovum"

replot_pearson = rbind(pearson_gal,pearson_sid,pearson_fer,pearson_sulfuri,pearson_sulfuro)
...
replot_pearson$x = factor(replot_pearson$x,levels = c("Temp","DO","pH","As","B","Ba","Co","Li","Mn","Na","Se","SO4"))
...
replot_pearson_plot <- ggplot(replot_pearson,
  ...
  scale_fill_manual(values = colorRampPalette(c(low, mid, high))(length(levels(replot_pearson$breaks))),
                    drop = FALSE) +
  ggtitle("Pearson coefficients for relevant genera and metadata") +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.y = element_text(face = "italic")) +
  guides(alpha = FALSE,
         fill = guide_legend(title = NULL))
# replot_pearson_plot_2 <- add_sub(replot_pearson_plot, x  = 0, hjust = 0, vjust = 3, size = 10,
#                                  substitute(paste("Temperature in Celsius\nDO in %")))
# replot_pearson_plot_3 <- add_sub(replot_pearson_plot_2, x  = 0, hjust = 0, vjust = 1.5, size = 10,
#                                  substitute(paste("Elements in ",mu, g.mL^{-1})))
```
```{r echo=FALSE, fig.align="center", fig.width=10}
replot_pearson_plot
```

So from *Pearson* correlations it looks like each of the relative abundance of each FeOB is defined by a specific set of metadata variables that probably drives co-occurrence across the dataset. **Gallionella** is very positively correlated to cobalt concentrations, less positively to manganese and arsenic and negatively to barium, temperature and sodium concentrations. The first two make sense since this genus is known to grow better a lower temperatures and has had its potential for manganese, cobalt and arsenic resistance reported [here](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3770913/).

Regarding **Sideroxydans**, it seems the only meaningful correlations point positively to cobalt and negatively to barium and boron. Again, at least some part of this may have been predicted by [previous work](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3770913/), but barium and boron aren't directly mentioned. Unlike **Gallionella**, **Sideroxydans** doesn't show correlations towards either temperature, dissolved oxygen or pH.

**Ferriphaselus** stands out as being negatively correlated to sulfate and sodium concentrations. Not sure what this means but sulfate isn't a direct indicator of elemental sulfur, sulfite or sulfide concentrations. Aside from that, weak negative correlations to lithium and temperature and positive correlations seen from cobalt.

A number of correlations arise with **Sulfuricurvum**, where it seems that it's strongly associated to boron and lithium concentrations, followed albeit in a weaker fashion by dissolved oxygen and pH. It's also negatively associated to the presence of cobalt.

**Sulfurovum** presence is marked by positive correlations (>0.45) towards temperature, pH, sodium, lithium, barium and boron. It's also strongly negatively correlated to cobalt presence.

Sodium seems to play a relatively important role across the dataset albeit no direct cause for this effect, from what I know. Sodium is the weakest ion to be adsorbed onto mineral surfaces and that may be the cause of its abundance, but how does it affect microbial communities?
